...
cat <<EOF > Thumbs_up_ID0000001-cont.sh
#!/bin/bash
set -e
# setting environment variables for job
export PATH=\$root_path
EOF
container_env /scratch >> Thumbs_up_ID0000001-cont.sh
cat <<EOF2 >> Thumbs_up_ID0000001-cont.sh
...
# do file integrity checks
pegasus-integrity --print-timings --verify=stdin 1>&2 << 'eof'
input.txt:tools-container
eof

printf "\n#########################[Container] Launching user task #########################\n"  1>&2

pegasus-kickstart  -n Thumbs_up -N ID0000001 -R condorpool  -s output.txt=output.txt -L add_thumbs -T 2020-07-16T03:23:22+00:00 /usr/bin/add_thumbs_up.py input.txt output.txt
set -e
EOF2


chmod +x Thumbs_up_ID0000001-cont.sh
...
set +e
job_ec=0
docker_init tools-container
job_ec=$(($job_ec + $?))

docker run --user root -v $PWD:/scratch -w=/scratch --entrypoint /bin/sh --name $cont_name  $cont_image -c "set -e ;export root_path=\$PATH ;if ! grep -q -E  "^$cont_group:" /etc/group ; then groupadd --gid $cont_groupid $cont_group ;fi; if ! id $cont_user 2>/dev/null >/dev/null; then useradd --uid $cont_userid --gid $cont_groupid $cont_user; fi; su $cont_user -c \"./Thumbs_up_ID0000001-cont.sh \""
job_ec=$(($job_ec + $?))
